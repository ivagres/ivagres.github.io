...
```markdown
1. Термин "личная информация", используемый в настоящем документе, определяется как любая информация, которая идентифицирует
...
```markdown
1. Доступ к апаратным частям устройства требуются для фиксации особенностей дефекта и его локализации.
```
